...
if v.Release < "(R2021a)"
    ...
    for i = 1:3
        ...
        set(ax(2),'Xlim',[1e-2 1e3])	  % 位相線図の横軸の範囲を設定
        set(ax(2),'Ylim',[-200 25])       % 位相線図の縦軸の範囲を設定
        set(ax(3),'Ylim',[-80 60])        % ゲイン線図の縦軸の範囲を設定
    ...
    for i = 4:6
        ...
        [~, hobj, ~, ~] = legend({'$k\sb{\rm P} = 1$','$k\sb{\rm P} = 5.75$','$k\sb{\rm P} = 20$'}, ...
